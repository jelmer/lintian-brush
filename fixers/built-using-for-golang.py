...
with control as updater:
    if any(
        iter_relations(updater.source.get("Build-Depends", ""), "golang-go")
    ):
        go_package = True
    if any(
        iter_relations(updater.source.get("Build-Depends", ""), "golang-any")
    ):
        go_package = True
    if not go_package:
        sys.exit(0)
...
if removed:
    report_result(
        "Remove unnecessary ${misc:Built-Using} for %s" % ", ".join(removed)
    )
