...
with RulesEditor() as editor:
    has_build_arch = bool(
        list(editor.makefile.iter_rules(b"build-arch", exact=False))
    )
    has_build_indep = bool(
        list(editor.makefile.iter_rules(b"build-indep", exact=False))
    )
    ...
    if any(
        line.lstrip(b" -").startswith(b"include ")
