...
def find_reverse_dependencies(udd, package):
    ...
    query = (
        ...
        + " FROM packages WHERE "
        + " OR ".join(
            [f"{field} LIKE CONCAT('%%', %s::text, '%%')" for field in fields]
        )
    )
        ...
        for row in cursor:
            for relname in fields:
                ...
                for option in parsed:
                    for rel in option:
                        if rel["name"] == package:
                            by_source.setdefault(row["source"], set()).add(
                                row["package"]
                            )
...
def find_dummy_transitional_packages(udd, release):
    ret = {}
    query = """\
SELECT package, description, depends FROM packages
WHERE release = %s AND description LIKE '%%transitional%%'"""
    with udd.cursor() as cursor:
        ...
        for row in cursor:
            ...
            if row[2] is None:
                ...
                if len(depends) != 1:
                    logging.debug(
                        "no single transition target for %s: %r",
                        row[0],
                        row[2],
                    )
...
def main(argv=None):
    ...
    transitions = find_dummy_transitional_packages(udd, args.release)
    ...
    if args.list_transitional_dummy:
        ...
        for dep in transitions:
            by_source = find_reverse_dependencies(udd, dep)
