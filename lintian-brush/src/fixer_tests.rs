...
fn run_fixer_testcase(fixer_name: &str, script_path: &Path, test_name: &str, path: &Path, tags: &[&str]) {
    ...
    fs_extra::dir::copy(path, &indir, &options).unwrap();

    let xfail_path = path.join("xfail");
    match std::fs::read_to_string(&xfail_path) {
        Ok(s) => {
            eprintln!("Skipping test {} because it is expected to fail: {}", test_name, s);
            return ();
        }
    ...
    env.insert("PYTHONPATH".to_owned(), {
        ...
        let mut path = pyo3::Python::with_gil(|py| {
            use pyo3::prelude::*;
            py.import_bound("sys").unwrap().getattr("path").unwrap().extract::<Vec<String>>()
        }).unwrap();
    ...
    let check_message = if !outdir.is_symlink() || outdir.read_link().unwrap() != PathBuf::from("in") {
        ...
        let expected_tags: HashSet<&str> = tags.to_vec().into_iter().collect();
