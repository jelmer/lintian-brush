...
fn versions_dict() -> HashMap<String, String> {
    ...
    versions.insert("deb-scrub-obsolete".to_string(), env!("CARGO_PKG_VERSION").to_string());
    versions.insert("breezy".to_string(), breezyshim::version::version().to_string());
    versions
}
...
fn main() -> Result<(), i32> {
    ...
    match check_clean_tree(&wt, &wt.basis_tree().unwrap(), &subpath) {
        ...
        Err(BrzError::WorkspaceDirty(..)) => {
            log::info!(
                "{}: Please commit pending changes first.", wt.basedir().display()
            );
            return Err(1);
        }
        Err(e) => {
    ...
    let compat_release = compat_release.unwrap_or_else(|| resolve_release_codename("oldstable", None).unwrap());
    ...
    if is_debcargo_package(&wt, &subpath) {
        report_fatal(
            versions_dict(), "nothing-to-do", "Package uses debcargo", None, None
        );
    ...
    let result = match scrub_obsolete::scrub_obsolete(
        ...
        Ok(r) => r,
        Err(scrub_obsolete::ScrubObsoleteError::EditorError(EditorError::FormattingUnpreservable(p, e))) => {
            for line in e.diff() {
                log::info!("{}", line);
            }
            report_fatal(
                ...
                &format!("unable to preserve formatting while editing {}", p.display()),
                None, None
            );
        }
        Err(scrub_obsolete::ScrubObsoleteError::EditorError(EditorError::GeneratedFile(p, e))) => {
            report_fatal(
                versions_dict(),
                "generated-file",
        ...
        Err(scrub_obsolete::ScrubObsoleteError::NotDebianPackage(_)) => {
            report_fatal(
                versions_dict(), "not-debian-package", "Not a Debian package.", None, None
            );
        }
        Err(scrub_obsolete::ScrubObsoleteError::EditorError(EditorError::TemplateError(p, e))) => {
            report_fatal(
                versions_dict(),
                "change-conflict",
        ...
        Err(scrub_obsolete::ScrubObsoleteError::SqlxError(e)) => {
            report_fatal(
                versions_dict(),
                "udd-error",
                &format!("Error communicating with UDD: {}", e), None, None
            );
        }
        Err(scrub_obsolete::ScrubObsoleteError::BrzError(e) | scrub_obsolete::ScrubObsoleteError::EditorError(EditorError::BrzError(e))) => {
            report_fatal(
                versions_dict(),
                "brz-error",
