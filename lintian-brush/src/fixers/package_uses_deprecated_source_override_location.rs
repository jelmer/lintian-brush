...
pub fn run(base_path: &Path) -> Result<FixerResult, FixerError> {
    ...
    Ok(FixerResult::builder("Move source package lintian overrides to debian/source.")
...
mod tests {
    ...
    fn test_simple_move() {
        ...
        assert_eq!(result.description, "Move source package lintian overrides to debian/source.");
    ...
    fn test_append_to_existing() {
        ...
        let content = fs::read_to_string(&new_path).unwrap();
        assert_eq!(content, "bar source: another-tag\nfoo source: some-tag exact match\n");
    }
