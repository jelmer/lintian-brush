...
def verify_watch_file(watch_file, source_package, expected_versions):
    ...
    for entry in getattr(watch_file, "entries", []):
        ret.append(
            verify_watch_entry(entry, source_package, expected_versions)
        )
    return ret
...
def main():  # noqa: C901
            ...
            with WatchEditor(
                ...
                if not args.force:
                    try:
                        ...
                        TemporaryWatchEntryVerficationError,
    ...
    if status is not None:
        logging.info(
            "Releases {} can be found with new watch entry {}".format(
                ", ".join(
                    sorted(list(status[0].releases.keys()), reverse=True)
                ),
                status[0].entry,
            )
        )
