...
fn remove_executable_bit(file_path: &Path) -> Result<bool, FixerError> {
    let metadata = fs::metadata(file_path)
        .map_err(|e| FixerError::Other(format!("Failed to get metadata for {}: {}", file_path.display(), e)))?;
    ...
    fs::set_permissions(file_path, new_perms)
        .map_err(|e| FixerError::Other(format!("Failed to set permissions for {}: {}", file_path.display(), e)))?;
...
mod tests {
    ...
    fn test_find_desktop_files() {
        ...
        assert!(desktop_files.iter().any(|p| p.file_name().unwrap() == "test.desktop"));
        assert!(desktop_files.iter().any(|p| p.file_name().unwrap() == "another.desktop"));
    }
