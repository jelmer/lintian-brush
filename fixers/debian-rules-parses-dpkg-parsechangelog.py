...
_KNOWN = [
    (b'dpkg-parsechangelog | sed -n -e \'s/^Version: //p\'',
     'DEB_VERSION'),
]
...
def drop_pkg_info_line(line):
    ...
    (variable, variable_matches,
     value_matches, hard) = is_pkg_info_var_line(line)
    ...
    if hard:
        ...
        fixed_lintian_tag(
            'source', 'debian-rules-parses-dpkg-parsechangelog',
...
try:
    ...
    for line in mf.contents:
        if (isinstance(line, bytes) and
                is_pkg_info_var_line(line)[1:3] == (True, True)):
