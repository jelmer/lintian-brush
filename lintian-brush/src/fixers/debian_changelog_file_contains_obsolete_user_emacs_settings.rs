...
pub fn run(base_path: &Path) -> Result<FixerResult, FixerError> {
    ...
    let re = Regex::new(
        r"(?s)(Local variables:.*?)add-log-mailing-address: .*\n(.*?End:)"
    ).unwrap();
    ...
    Ok(FixerResult::builder("Drop no longer supported add-log-mailing-address setting from debian/changelog.")
        .fixed_tags(vec!["debian-changelog-file-contains-obsolete-user-emacs-settings"])
...
mod tests {
    ...
    fn test_remove_add_log_mailing_address() {
        ...
        assert_eq!(result.description, "Drop no longer supported add-log-mailing-address setting from debian/changelog.");
