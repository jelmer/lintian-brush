        ...
        if binary.section().is_none() {
            ...
            if let Some(section) = find_expected_section(&regexes, &package_name) {
                ...
                let issue = LintianIssue {
                    ...
                    info: Some(vec![format!("(in section for {}) Section [debian/control:{}]", package_name, line_no)]),
                };
    ...
    if binary_sections.len() == 1 {
        ...
        if let Some(mut source) = editor.source() {
            ...
            let issue = LintianIssue {
                ...
                tag: Some("recommended-field".to_string()),
                info: Some(vec![format!("(in section for source) Section [debian/control:{}]", source_line)]),
            };
